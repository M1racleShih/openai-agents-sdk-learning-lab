...
<p class="lesson-kicker">M03 · 75 minutes · concepts + lab</p>
...
<div class="lesson-meta" aria-label="Lesson information">
  ...
  <span>5 external statuses</span>
  <span>6 abnormal scenario groups</span>
</div>
...
- represent caller-visible results with `completed`, `incomplete`, `failed`, `cancelled`, and `timed_out`;
...
- cover missing sources, tool failure, model failure, timeout, cancellation, and incomplete results.

!!! abstract "Chapter boundary"

    This chapter adds external status and failure wrapping to one run. It does not add retries,
    Sessions, persistence, or a general exception framework. M04 covers traces and redacted
    records; M05 connects these semantics to streaming and SDK cancellation.
...
types include `MessageOutputItem`, `ToolCallItem`, and `ToolCallOutputItem`. Do not make an application
...
`ModelBehaviorError`, and `ToolTimeoutError` all inherit from `AgentsException`. In `v0.20.0`, the
...
This course uses these external statuses. They are an application protocol, not SDK exception names:
...
| `cancelled` | The caller explicitly stopped the run | Stable code `CANCELLED`; no successful answer |
| `timed_out` | A tool or the run exceeded its published time boundary | `TOOL_TIMEOUT` or `RUN_TIMEOUT`; never masquerades as `completed` |
...
`Runner.run(..., max_turns=6)` allows at most 6 model calls. `v0.20.0` defines a turn as one model
...
Do not pass `max_turns=None` to this bounded use case because that disables the turn limit.
...
| An asynchronous tool exceeds its own timeout | `timed_out` | `TOOL_TIMEOUT` |
| The complete run exceeds wall-clock time | `timed_out` | `RUN_TIMEOUT` |
| The caller cancels the active run | `cancelled` | `CANCELLED` |
...
In `v0.20.0`, `Runner.run` accepts `error_handlers`. The `"max_turns"` and
...
Keep one final general exception exit so the application use case can return a machine-readable
failure. Log the stack locally, but expose only a fixed code and safe message. SDK and provider
wording can change, so exception strings are never a stable protocol. Do not return exception
...
tool or run timeout → status == "timed_out" with separate stable codes
explicit cancellation → status == "cancelled" and errors contains CANCELLED
incomplete result → status == "incomplete" and retained answer/evidence still satisfies the result model's consistency rules
```

Also verify that Pydantic rejects these classes of object: `completed` with an error, a
failed/cancelled/timed-out result with a domain answer, and a non-completed status without an error. Trigger the `max_turns` and
...
4. What is the main difference between `incomplete` and `failed`, `cancelled`, or `timed_out`?
...
10. Why should abnormal-path tests assert `WorkerResult` before matching SDK exception or log text?
...
4. `incomplete` has a trustworthy partial answer or evidence. The other three have no deliverable
   successful answer and distinguish failure, caller cancellation, and an exceeded time boundary.
...
1. Add a required `status` to `WorkerResult`, allowing only `completed`, `incomplete`, `failed`,
   `cancelled`, and `timed_out`.
2. Define every entry in `errors` as a completion-blocking problem. Add deterministic validation:
   `completed` has no error; every non-completed status has at least one error; failed, cancelled,
   and timed-out results have no answer or evidence.
...
7. Handle `ToolTimeoutError`, run-level `TimeoutError`, `asyncio.CancelledError`, other
   `ModelBehaviorError` cases, and a final unknown exception separately. Expose only stable codes
   and safe messages.
8. Without calling a real model, cover six groups: missing source, tool failure, model failure,
   timeout, cancellation, and a valid but incomplete result. In the timeout group, assert both
   `TOOL_TIMEOUT` and `RUN_TIMEOUT`.
...
- missing source, tool failure, model failure, timeout, cancellation, and incomplete results each
  produce a stable, assertable `WorkerResult`;
...
- an application caller can distinguish completion, incompleteness, failure, cancellation, and
  timeout by reading only `status` and `errors`;
...
- [`v0.20.0` Running agents: agent loop, turn limits, and errors](https://github.com/openai/openai-agents-python/blob/v0.20.0/docs/running_agents.md)
- [`v0.20.0` Results: final output and new items](https://github.com/openai/openai-agents-python/blob/v0.20.0/docs/results.md)
- [`v0.20.0` Runner and turn-limit source](https://github.com/openai/openai-agents-python/blob/v0.20.0/src/agents/run.py)
- [`v0.20.0` SDK exception source](https://github.com/openai/openai-agents-python/blob/v0.20.0/src/agents/exceptions.py)
- [`v0.20.0` run error handler source](https://github.com/openai/openai-agents-python/blob/v0.20.0/src/agents/run_error_handlers.py)
- [`v0.20.0` structured-output validation source](https://github.com/openai/openai-agents-python/blob/v0.20.0/src/agents/agent_output.py)
- [`v0.20.0` function-tool timeout source](https://github.com/openai/openai-agents-python/blob/v0.20.0/src/agents/tool.py)
- [`v0.20.0` Agent lifecycle example](https://github.com/openai/openai-agents-python/blob/v0.20.0/examples/basic/agent_lifecycle_example.py)
...
documentation and lifecycle example; add the course's five-status rules, 6-turn limit,
