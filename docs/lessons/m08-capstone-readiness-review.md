...
<p class="lesson-deck">把前八章连成一个用户直接使用的最小应用；证明每层状态、数据边界和失败语义都成立。</p>
...
- 用公开合成设备维护资料演示批准清单、provenance 与按需只读查询；
...
批准清单中的最少资料并调用只读工具。缺资料时必须诚实返回 `incomplete`，不能补造记录。
...
| tools | 批准清单内只读读取与 provenance | 写操作、任意命令、凭据外泄 |

SDK Session 是对话连续性存储；Trace 是运行路径；RunOutcome 是调用方协议；终端状态是当前
presentation adapter 的生命周期。任何一个都不能代替另外三个。
...
| 层级 | 输入 | 关键断言 |
| --- | --- | --- |
| contract/unit | 固定模型对象 | 五种状态一致性规则、provenance、事件 union |
| tool | fixture + fake subprocess/service | 工具 allowlist、无 shell、超时、输出上限、checksum |
...
- `completed` 仍缺资料：检查应用的资料覆盖规则，不要相信模型自报状态；
